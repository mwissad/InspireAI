...
/** Matches server auto-publish (`NOTEBOOK_DEST`) — avoids non-existent `/Users/.../inspire_ai` folders. */
const DEFAULT_WORKSPACE_NOTEBOOK_PATH = '/Shared/inspire_ai';
...
export default function ConfigPage({ settings, update, onConfigured }) {
  ...
  // Publish DBC
  const publish = async () => {
    const destPath = publishPath || DEFAULT_WORKSPACE_NOTEBOOK_PATH;
    if (!destPath) return;
          ...
          <div className="px-5 py-4">
            <div className="relative">
              ...
              <input
                ...
                onChange={(e) => {
                  update('databricksHost', normalizeDatabricksHost(e.target.value));
                  setTokenStatus(null);
                }}
          ...
          <div className="px-5 py-4">
            ...
            <div className="flex items-center gap-3">
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  value={publishPath || notebookPath || DEFAULT_WORKSPACE_NOTEBOOK_PATH}
                  onChange={(e) => setPublishPath(e.target.value)}
                  className="w-full pl-9 pr-3 py-2 text-sm border border-border rounded-md bg-surface text-text-primary placeholder:text-text-tertiary glow-focus transition-smooth"
                  placeholder={DEFAULT_WORKSPACE_NOTEBOOK_PATH}
                />
      </div>
      <button
                onClick={publish}
